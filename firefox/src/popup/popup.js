...
import {
  filterReflections,
  sortReflections,
} from "../core/js/logic/reflections.js";
  ...
  if (enabledReminders.length === 0) {
    myRemindersList.innerHTML = `
            <div class="empty-state">
                <div class="empty-state-icon"><i class="fa-solid fa-bars"></i></div>
  ...
  if (isActive) {
    return `<div class="card-header">
            <div class="card-content">
                <div class="card-title">${bookmarkBadge}${name || description}</div>
                <div class="card-description">${timeLabel}</div>
            </div>

            <div class="card-header-actions">
                <button class="btn-onlyicon btn-secondary calendar-btn" title="سجل الإنجاز">
                <i class="fa-solid fa-calendar"></i>
                </button>
                <button class="switch" role="switch" aria-checked="${isActive && enabled !== false}" data-id="${id}">
        ...
        <div class="card-actions">
            ${checkboxHtml}
            <div class="btn-group">
                <button class="btn-onlyicon btn-destructive delete-btn"><i class="fa-solid fa-trash"></i></button>
                <button class="btn-onlyicon btn-outline edit-btn"><i class="fa-solid fa-pen"></i></button>
    ...
    <div class="card-actions-col">
        <div class="actions-row">
            ...
            <button class="switch"
                role="switch"
                aria-checked="${isActive && enabled !== false}"
                data-id="${id}">
                <span class="switch-thumb"></span>
            </button>
        </div>
...
function createReminderCard(
  ...
  const bookmarkBadge = hasBookmark
    ? '<span style="margin-right:0.5rem; font-size:1rem;" title="لديك علامة محفوظة">🔖</span>'
    : "";
  ...
  //             ${checkboxHtml}
  //             <div class="btn-group">
  //                 <button class="btn-onlyicon btn-destructive delete-btn" ${!isActive ? 'style="display:none"' : ""}><i class="fa-solid fa-trash"></i></button>
  //                 <button class="btn-onlyicon btn-outline edit-btn" ${!isActive ? 'style="display:none"' : ""}><i class="fa-solid fa-pen"></i></button>
  //                 <button class="btn btn-primary read-btn">اقرأ</button>
  //             </div>
  //         </div>
  //     `;

  div.innerHTML = GetReminderCard(
    reminder.id,
    isActive,
    bookmarkBadge,
    reminder.name,
  ...
  div.querySelector(".read-btn").addEventListener("click", () => {
    const url = api.runtime.getURL(
      `src/reader/reader.html?reminderId=${reminder.id}`,
    );
    api.tabs.create({ url });
  });
...
function startEditing(reminder) {
  ...
  // Update tab text
  if (tabs[1]) tabs[1].innerHTML = `<i class="fa-solid fa-pen"></i>`;
...
function resetForm() {
  ...
  // Reset tab text
  if (tabs[1]) tabs[1].innerHTML = `<i class="fa-solid fa-plus"></i>`;
...
function validateAyahRange() {
  ...
  if (start > end) {
    ayahRangeError.textContent =
      "آية البداية يجب أن تكون أقل من أو تساوي آية النهاية";
...
async function importData(file) {
  try {
    ...
    const { user_reminders, read_history, bookmarks, quran_reflections } =
      importObj.data;
    ...
    const errorMsg =
      err instanceof SyntaxError ? "الملف ليس بتنسيق JSON صحيح." : err.message;
...
async function resetExtension() {
  try {
    ...
    showAlert(
      "تم إعادة الضبط",
      "تم إعادة ضبط الإضافة بنجاح. سيتم تحديث الصفحة.",
    );
...
function showConfirmModal(title, message, onConfirm) {
  ...
  confirmBtn.onclick = async () => {
    ...
    modalDescription.textContent =
      "هل أنت متأكد من رغبتك في حذف هذا التذكير؟ لا يمكن التراجع عن هذه الخطوة.";
    await onConfirm();
  };
...
async function loadReflectionsJournal(filter = "") {
  ...
  for (const ref of filtered) {
    ...
    const openInMushaf = () => {
      const url = api.runtime.getURL(
        `src/reader/reader.html?surahId=${ref.surah}&targetAyah=${ref.ayah}`,
      );
      api.tabs.create({ url });
    };
