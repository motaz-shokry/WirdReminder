// app.js - Full Extension Demo (Landing Page)
// Uses the EXACT same logic patterns as the extension's popup.js

import { fetchAllSurahs, fetchSurahVerses, fetchAyahRange, fetchJuzVerses, getSurahName, storage } from './api.js';
import { parseVersesToPages } from './parser.js';

// ============================================
// PRESETS DATA (Same as extension's presets.json)
// ============================================
const presets = [
    {
        id: 'kahf_friday',
        ...
        type: 'surah',
        target: { surahId: 18 },
        timing: { frequency: 'weekly', day: 5, time: '10:00' },
        defaultEnabled: false
    },
    {
        id: 'mulk_night',
        name: 'سورة الملك',
        description: 'قراءة سورة الملك كل ليلة',
        type: 'surah',
        target: { surahId: 67 },
        timing: { frequency: 'daily', time: '21:00' },
        defaultEnabled: false
    },
    {
        id: 'baqarah_ends',
        name: 'خواتيم البقرة',
        description: 'آخر آيتين من سورة البقرة قبل النوم',
        type: 'ayah_range',
        target: { surahId: 2, startAyah: 285, endAyah: 286 },
        timing: { frequency: 'daily', time: '22:00' },
        defaultEnabled: false
    }
];
...
async function loadAllReminders() {
    const user_reminders = storage.get('user_reminders') || [];
    const read_history = storage.get('read_history') || [];
...
function isReadInCurrentPeriod(reminder, lastReadTs) {
    ...
    if (freq === 'daily') {
        ...
        const scheduledDay = reminder.timing.day ?? 5;
        const resetTime = new Date(now);
        resetTime.setHours(hours, minutes, 0, 0);
...
function getFrequencyLabel(timing) {
    ...
    if (timing.frequency === 'weekly') return 'أسبوعياً';
    return '';
}
...
async function addReminder(reminder) {
    const list = storage.get('user_reminders') || [];
    if (!list.find(r => r.id === reminder.id)) {
        list.push({ ...reminder, enabled: true });
        storage.set('user_reminders', list);
    }
}
...
async function removeReminder(id) {
    const list = storage.get('user_reminders') || [];
    storage.set('user_reminders', list.filter(r => r.id !== id));
}
...
async function addReadMark(reminderId, reminderName) {
    const history = storage.get('read_history') || [];
    history.push({ reminderId, reminderName, timestamp: Date.now() });
    storage.set('read_history', history.slice(-1000));
}
...
async function removeReadMark(reminderId) {
    ...
    if (lastIndex !== -1) {
        ...
        storage.set('read_history', history);
    }
}
...
function saveBookmark(verseKey, wordPosition) {
...
function initSettingsListeners() {
    ...
    document.getElementById('export-data').addEventListener('click', () => {
        const data = {
            user_reminders: storage.get('user_reminders') || [],
            read_history: storage.get('read_history') || [],
    ...
    importFile.addEventListener('change', async (e) => {
        ...
        try {
            ...
            if (data.user_reminders) storage.set('user_reminders', data.user_reminders);
            if (data.read_history) storage.set('read_history', data.read_history);
